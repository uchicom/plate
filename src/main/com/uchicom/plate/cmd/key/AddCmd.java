...
/**
 * Key(別名)を追加するコマンド key, classPath, [static method]を引数に指定する。 static
 * methodを指定しなかった場合はmainが呼び出される。
 * 
...
public class AddCmd extends AbstractCmd {
	...
	public boolean execute(CmdSocketHandler handler, String[] params) {
		if (params.length == 2) {
			return broker.getMain().addKey(params[0], params[1], 
					handler.getCurrentPort());
		} else if (params.length == 3) {
			return broker.getMain().addKey(params[0], params[1], params[2],
					handler.getCurrentPort());
		}
		return false;
	}
